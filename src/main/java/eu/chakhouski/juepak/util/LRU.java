...
public final class LRU<K,V> extends LinkedHashMap<K,V> implements Map<K, V>
{
     ...
     * Debug info item.
     */
    private static final class LRUHitMissInfo
    {
        ...
        {
            final long totalGets = LRU_DEBUG_hits + LRU_DEBUG_misses;

            if (totalGets > 0)
            {
                final double hitRate = (double) LRU_DEBUG_hits / (totalGets);
     ...
     * ! DEBUG ONLY
     *
     * Maps each LRU onto a special {@link LRU.LRUHitMissInfo} debug info.
     * Used to minimize memory overhead if debugging is disabled (normal conditions).
     */
    private static final Map<LRU<?, ?>, LRUHitMissInfo> lruDebugInfo =
            LRU_DEBUG_ENABLED ? new ConcurrentHashMap<>() : Collections.emptyMap();
    ...
    public LRU(int sizeLimit)
    {
        ...
        if (LRU_DEBUG_ENABLED)
        {
            ...
            lruDebugInfo.put(this, new LRUHitMissInfo());
        }
    ...
    private static void finalizeAll()
    {
        for (final Entry<String, LRU<?, ?>> entry : allLRU.entrySet())
        {
            ...
            final LRUHitMissInfo hm = lruDebugInfo.get(lru);

            final float hitRate = hm.getHitRatePercent();

            System.out.println(String.join(System.lineSeparator(),
                    "Cache rate for \"" + entry.getKey() + "\" LRU",
                    " > Hits: " + hm.LRU_DEBUG_hits + ", Misses: " + hm.LRU_DEBUG_misses,
